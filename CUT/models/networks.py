...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        return_ids = []
        return_feats = []
        # print('feats.shape',len(feats)) #feats.shape 5
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
        for feat_id, feat in enumerate(feats):
            # print('feat.shape',feat.shape)#feat.shape torch.Size([4, 3, 262, 262])
            #([4, 128, 256, 256])
            #[4, 256, 128, 128])
            #4, 256, 64, 64]
            ...
            if num_patches > 0:
                if patch_ids is not None:
                    ...
                    # print('int(min(num_patches, patch_id.shape[0]))',int(min(num_patches, patch_id.shape[0])))
                    # print('patch_id.shape[0]',patch_id.shape[0])
